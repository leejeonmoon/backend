...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public static final String REDIRECT_PATH = "/oauthIndex";
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        String refreshToken = tokenProvider.generateToken(user, REFRESH_TOKEN_DURATION); // 생성
        saveRefreshToken(user.getMemberId(), refreshToken); // 저장
        addRefreshTokenToCookie(request, response, refreshToken); // 쿠키에 저장
        // 액세스 토큰 생성 -> 패스에 액세스 토큰 추가
        String accessToken = tokenProvider.generateToken(user, ACCESS_TOKEN_DURATION); // 생성
        String targetUrl = getTargetUrl(accessToken); // 추가
        // 인증 관련 설정값, 쿠키 제거
